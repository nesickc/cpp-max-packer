...
For each feature, first identify the governing requirement IDs and observable acceptance scenario. Update the spec/contracts before implementing a behavior change. Implement the smallest complete vertical slice, run the relevant checks, and record evidence in the change description. A requirement is complete only when its acceptance evidence exists; a screenshot alone does not establish packing validity.
